...
int comedi_subdevice = 1;

int comedi_rate = 20480; /* XXX set this to max valid upon open */
...
static int start_comedi_running(void)
{
	...
	if (ret >= 0) {
		...
		comedi_running = 1;
		//   fprintf(stderr,"cmd passed %i    \n",ret);

	} else {
		fprintf(stderr, "cmd failed %i    \n", ret);
		comedi_error = comedi_errno();
	}
...
static int open_comedi(void)
{
	...
	//    fprintf(stderr,"open comedi devname \n");
	...
	if (comedi_board_name && strncmp(comedi_board_name, "DAQP", 4) == 0) {
		...
		if (ret == -1) {
			fprintf(stderr, "generic timed failed  \n");
			comedi_error = comedi_errno();
			/* close_comedi(); */
			return 0;
		}
	...
	if (start_comedi_running() < 0) {
		fprintf(stderr, "start_comedi_runnin 0 \n");
		return 0;
	} else {
		fprintf(stderr, "start_comedi_runnin 1  \n");
		return 1;
	}
